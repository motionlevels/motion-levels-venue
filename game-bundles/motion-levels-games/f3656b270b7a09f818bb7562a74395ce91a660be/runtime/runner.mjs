...
function normalizeGameConfig(config, manifest6) {
  return {
    seed: normalizeGameSeed(config.seed),
    playerCount: normalizePlayerCount(config.playerCount, manifest6),
    players: Array.isArray(config.players) ? config.players : [],
    durationMillis: normalizeNonNegativeNumber(config.durationMillis, manifest6.defaultDurationMillis),
    nowMillis: normalizeNonNegativeNumber(config.nowMillis, 0),
    difficulty: normalizeGameDifficulty(config.difficulty, manifest6),
    options: normalizeGameConfigOptions(config.options, manifest6)
  };
}
...
function normalizePlayerCount(value, manifest6) {
  const rounded = typeof value === "number" && Number.isFinite(value) ? Math.round(value) : defaultGamePlayerCount(manifest6);
  if (manifest6.players.allowAny === true && rounded === 0) {
    return 0;
  }
  return clamp(rounded, manifest6.players.min, manifest6.players.max);
}
function defaultGamePlayerCount(manifest6) {
  return manifest6.players.allowAny ? 0 : manifest6.players.min;
}
function normalizeNonNegativeNumber(value, fallback) {
  return typeof value === "number" && Number.isFinite(value) ? Math.max(0, value) : fallback;
}
function gameDifficultyOptions(manifest6) {
  const configured = manifest6.config?.difficulty?.options;
  return configured?.length ? [...configured] : [...DEFAULT_GAME_DIFFICULTIES];
}
function normalizeGameDifficulty(value, manifest6) {
  const options = gameDifficultyOptions(manifest6);
  const configuredDefault = manifest6.config?.difficulty?.default;
...
function normalizeGameConfigOptions(options, manifest6) {
  const source = options ?? {};
  return Object.fromEntries(
    (manifest6.config?.vars ?? []).map((configVar) => [
      configVar.key,
...
// packages/runner/src/runner.ts
var sourceRevision = true ? "f3656b270b7a09f818bb7562a74395ce91a660be" : "development";
